...
const CHANNEL_ID =
  process.env.ENVIRONMENT === "production" ? "C57UPMZLY" : "C05EG79V8HW";

const API_URL =
  process.env.ENVIRONMENT === "production"
    ? "http://api.cloud-platform-hammer-bot.svc.cluster.local:3001"
    : "https://hammer-bot.live.cloud-platform.service.justice.gov.uk";
...
const postFail = async (data, ts) => {
  ...
  const failed = data.some((pr) =>
    pr.InvalidChecks.length
      ? pr.InvalidChecks.some((check) => check.Status === 1)
      : false,
  );
...
const retryLater = (ids, ts, retryInMs) => {
  setTimeout(async () => {
    ...
    await postReply(
      "It looks like checks on your pr are _still_ pending even after waiting a while. A Cloud Platform team member will come and take a look.",
      ts,
    );
...
const postPendingRecent = async (data, ts, ids) => {
  const pendingRecent = data.filter((pr) =>
    pr.InvalidChecks.length
      ? pr.InvalidChecks.some(
          (check) => check.Status === 2 && check.RetryInNanoSec > 0,
        )
      : false,
  );

  if (pendingRecent.length && pendingRecent.length > 0) {
    const retryIn = pendingRecent
      .map((pr) => pr.InvalidChecks.map((check) => check.RetryInNanoSec))
      .flat()
      .sort((a, b) => a - b)[0];
...
app.message(
  "github.com/ministryofjustice/cloud-platform-environments/pull/",
  async ({ message }) => {
    ...
    const pendingOlderThan10Mins = data.some((pr) =>
      pr.InvalidChecks.length
        ? pr.InvalidChecks.some(
            (check) => check.Status === 2 && check.RetryInNanoSec === 0,
          )
    ...
    if (pendingOlderThan10Mins) {
      ...
      retryLater(ids, message.ts, WAIT_MSECS);
    }
  },
);
